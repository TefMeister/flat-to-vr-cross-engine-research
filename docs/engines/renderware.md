# RenderWare

*One page per engine family this account has at least one conversion project on. This page holds
the **shared, cross-game truth** for the family; everything game-specific lives in each project's
`ENGINE-DOSSIER.md`, linked below. The [engines index](../engines-index.md) has the one-line
orientation row. Curated by the cross-project research sweep.*

## Identity

- **Engine:** Criterion Software's RenderWare — the widely licensed 2000s middleware engine
  (used across many publishers' titles of the era).
- **Render API:** Direct3D 8/9 era; see the dossier for what the Manhunt PC build actually uses.
- **Known public VR path:** none turnkey. Manual build; for anything Direct3D 8 or older, a
  D3D8-to-modern wrapper comes first (see the engines index's last row).

## Our projects on this engine

| Game | Engine dossier | All project repos |
| --- | --- | --- |
| Manhunt (2003) | [`ENGINE-DOSSIER.md`](https://github.com/TefMeister/manhunt-2003-vr-engine-research/blob/main/ENGINE-DOSSIER.md) | [`manhunt-2003-vr-*`](https://github.com/TefMeister?tab=repositories&q=manhunt-2003-vr) |

## Shared findings

*Seeded 2026-08-26; grown by the research sweep as cross-project truths emerge. Nothing has been
generalised up to this page yet — the per-project dossiers linked above are the current source of
truth for this family.*

## See also

- [engines index](../engines-index.md) — the "Bespoke / older custom engines" and "Anything
  Direct3D 8 or older" rows.
- [Generic drivers for older D3D9 games](../generic-drivers/) — the vorpX/geo-11/dgVoodoo2
  routes relevant to this era.
